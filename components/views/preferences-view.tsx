...
export function PreferencesView() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Still saved locally via Zustand persist — just mark saved
      setSaveStatus('saved')
    } finally {
            ...
            <div className="grid grid-cols-2 gap-4">
              {bodyTypeOptions.map((type) => (
                <button
                  ...
                  className={cn(
                    "p-4 rounded-3xl transition-all duration-300 group cursor-pointer border flex flex-col items-center text-left",
